...
export default function Bride({ data }) {
  const _renderDecoration = () => {
    return (
      <>
        <motion.div className="absolute bottom-0 w-screen max-w-screen-sm">
          <Mosque className="h-auto w-full text-[#2e2a61]" />
        </motion.div>

        <motion.div className="absolute -top-10 right-8 w-8">
          <Lamp1 className="h-auto w-full text-[#2e2a61]" />
        </motion.div>
        <motion.div className="absolute -top-16 right-32 w-8">
          <Lamp1 className="h-auto w-full text-[#2e2a61]" />
        </motion.div>
      </>
    )
  ...
  return (
    <section className="relative flex flex-col items-center justify-center gap-4 overflow-hidden px-6 py-24">
      <motion.div
        whileInView="visible"
        viewport={{ once: true }}
        initial="hidden"
